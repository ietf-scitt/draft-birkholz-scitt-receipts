...
6. Extract the Issued At parameter from sign_protected of the leaf_info field of the receipt contents.

7. Verify the signature with the root hash as payload using the certificate chain established by the node certificate embedded in the receipt and the service certificate part of the service's parameters (see {{parameters}}) using the Issued At time to verify certificate validity periods.
