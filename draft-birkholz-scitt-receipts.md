...
Certificate: The X.509 certificate used as trust anchor to verify signatures generated by the log.
...
- x5chain: X.509 certificate chain as an array of DER-encoded X.509 certificates, excluding the certificate declared in the log's parameters. The first certificate must be the one whose private key was used to compute the signature. An empty array means that there is no certificate hierarchy.
...
6. Create a Receipt structure and fill it with SIGNATURE, the signing certificate hierarchy excluding the certificate declared in the log's parameters, the inclusion proof, and the LeafInfo.
...
6. Verify the signature with the root hash as payload using the certificate chain included in the receipt and the declared certificate from the log's parameters (see {{parameters}}).
