...
- Signature Algorithm: The ECDSA signature algorithm used to sign the Merkle tree root (see {{sig-alg-registry}}).
...
All definitions in this section use the hash algorithm required by the signature algorithm set in the TS parameters (see Section {{parameters}}). We write HASH to refer to this algorithm, and HASH_SIZE for the fixed length of its output in bytes.
